...
class DungeonMap:
    ...
    def getFlattenedBranches(self) -> List[DungeonNode]:
        def difficulty(x): return x.difficulty
    ...
    def _getDuplicates(self, key):
        ...
        for node in branches:
            def isMatch(x): return getattr(x, key) == getattr(node, key)
            def isNotSelf(x): return x != node
            def isDuplicate(x): return isMatch(x) and isNotSelf(x)
...
class Dungeon:
    def __init__(self, playHours=4, timeScale=15):
        ...
        if (self.maxDifficulty <= 0):
            raise Exception(
                "Can not calculate parameters given, please change and try again.")
        if(playHours * timeScale > 105):
            ...
            raise Exception(
                "Time and Scale must total out to less than 105 minutes.")
    ...
    def getDiagram(self):
        ...
        for exit in set(exits):
            output.append(f'\t{exit}')
        return "\n".join(output)

    def getExpectedPlayTime(self):
        totalTime = 0
        for branch in self.branches.branches:
            for node in branch:
                totalTime = totalTime + (self.timeScale * node.difficulty)
        return f'Expected Play Time: {totalTime/60}'
...
class DungeonBot(Dungeon):
    def printEncounters(self):
        def _getNumberAsEmoji(number):
            ...
            numberString = ''.join(
                map(lambda x: numberEmoji[x], list(str(number))))
            return numberString
        ...
        for node in branches:
            ...
            encounter = []
            encounter.append(
                f'**{node.name}** *Level {node.difficulty} {node.theme}*')
            encounter.append('')
            encounter.append(f'**Countdown** {node.countdown}')
            encounter.append(
                f'**Alarm** {node.alarm} *( :x: to activate alarm.)*')
            encounter.append(
                f'**Lockbox** {node.lock} *( :white_check_mark:  to clear tile.)*')
            encounter.append('')
            encounter.append(
                f'**Threats** {node.getThreatSummary()} *(:boom: to delete contents.)*')
            if(hasattr(node, 'benefit')):
                encounter.append(
                    f'**Benefit** {node.benefit} *(:ribbon: to collect benefit!)*')
